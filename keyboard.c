...
void TASK_SCAN_RELEASE(void)
{
    // —читываем состояние портов клавиатуры колонки и строки
    register unsigned char KEYS = KEY_P0 - (KEY_P1|(~MASK_KEY_P1));
    if(!KEYS)
    {
        OS.FLAGS.KEYBRD = 1; // Клавиатура неактивна - сообщим ќ— о том что клавиатура выдала код
    }
    else
    {
        TIME_UpTask(sSCANR);
...
Returns:  none
    ...
    Код клавиши обработан ќ—, разрешаем сканирование далее
*************************************************************************/
